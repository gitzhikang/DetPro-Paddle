...
from .utils import new_tensor,repeat
...
def bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
    ...
    proposals = proposals.astype('float32')
    gt = gt.float('float32')
    ...
    means = new_tensor(means,deltas).unsqueeze(0)
    stds = new_tensor(stds,deltas).unsqueeze(0)
...
def delta2bbox(rois,
    ...
    """
    means = repeat(new_tensor(means,deltas).reshape([1, -1]),[1, deltas.shape[1] // 4])
    # stds = deltas.new_tensor(stds).view(1, -1).repeat(1, deltas.size(1) // 4)
    stds = repeat(new_tensor(stds, deltas).reshape([1, -1]), [1, deltas.shape[1] // 4])
    try:
    ...
    dw = dw.clip(min=-max_ratio, max=max_ratio)
    dh = dh.clip(min=-max_ratio, max=max_ratio)
    ...
    bboxes = paddle.stack([x1, y1, x2, y2], axis=-1).reshape(deltas.shape)
    return bboxes
