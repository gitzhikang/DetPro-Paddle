...
class BitmapMasks(BaseInstanceMasks):
    ...
    def crop_and_resize(self,
        ...
        fake_inds = arange(
            num_bbox, place=device).cast(dtype=bboxes.dtype)[:, None]
        rois = paddle.concat([fake_inds, bboxes], axis=1)  # Nx5
        # rois = rois.to(device=device)
        rois = paddle.to_tensor(rois.numpy(),place=device)
        if num_bbox > 0:
            # gt_masks_th = torch.from_numpy(self.masks).to(device).index_select(
            #     0, inds).to(dtype=rois.dtype)
            gt_masks_th = paddle.to_tensor(self.masks,place=device).index_select(axis=0,index=inds).cast(rois.dtype)
            # targets = roi_align(gt_masks_th[:, None, :, :], rois, out_shape,
            #                     1.0, 0, 'avg', True).squeeze(1)
            targets = paddle.vision.ops.roi_align(gt_masks_th[:, None, :, :],rois,rois.shape[0],out_shape,1.0,0,True)
    ...
    def to_tensor(self, dtype, device):
        ...
        return paddle.to_tensor(self.masks, dtype=dtype, place=device)
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def to_tensor(self, dtype, device):
        ...
        if len(self.masks) == 0:
            ...
            #                    dtype=dtype,
            #                    device=device)
            return paddle.to_tensor(np.empty((0, self.height, self.width)),dtype=dtype,place=device)
        ndarray_masks = self.to_ndarray()
        return paddle.to_tensor(ndarray_masks, dtype=dtype, place=device)
