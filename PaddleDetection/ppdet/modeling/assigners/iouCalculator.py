import paddle
import numpy as np
from ..heads.file.roiheads.utils import new_tensor
...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    if rows * cols == 0:
        if is_aligned:
            return paddle.to_tensor(np.zeros(shape=batch_shape+[rows, ]),dtype=bboxes1.dtype,place=bboxes1.place)
        else:
            # return bboxes1.new(batch_shape + (rows, cols))
            return paddle.to_tensor(np.zeros(shape=batch_shape + [rows, cols]),dtype=bboxes1.dtype,place=bboxes1.place)
